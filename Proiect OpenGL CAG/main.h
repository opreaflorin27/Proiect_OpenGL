...
double time_now() { // returns how many seconds since the first call to time_now()
...
idle(void)
{
    ...
    double now = time_now();
    if (now - last_update < 1.0 / 120) return; // 60 = de cate ori pe secunda o sa ruleze ce e mai jos
    last_update = now;
    if (spinning)
        click++;
    switch (state) {
